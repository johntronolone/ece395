...
class Topology:
	def __init__(self,input_size):
		...
		self.min_HL = 8
		self.epochs = 150 #Number of epochs the ANN is trained for - 300 should be sufficient
		...
		for i in range(1,5):
			self.W_fc[i] = self.getWeightVariable([self.fc[i - 1],self.fc[i]],'W_fc' + str(i + 1))
		self.W_fc[5] = self.getWeightVariable([self.fc[4], self.output_size],'W_fc6')
...
class ANNeSynth:
	...
	def loadDataSet(self):
		...
		filename = 'all_one_scale_frames.npy'#self._corpus	#Static Data used for training net
		data_path = os.path.join(os.getcwd(),filename)
		self.frames = np.load(data_path)
		self.frames = np.asarray(self.frames)
		self.validate = self.frames
	...
	def makeTensorFlowLayers(self):
		...
		initialLayer = tf.nn.relu(tf.add(tf.matmul(self.x_, self.topology.W_fc[0]),0))
		initialLayer2 = tf.nn.relu(tf.add(tf.matmul(self.controller, self.topology.W_fc[3]),0))
		self.modulators = tf.placeholder(tf.float32, shape=[None, self.topology.fc[2]])
	...
	def trainNeuralNetwork(self):
		...
		loss = loss_lookup[self._loss_choice]+self.topology.l2_lamduh*l2 #Imposing L2 penalty
	...
	def execute_old(self,values,n_frames=150,filename='loop'):
	...
	def execute(self,values,n_frames=150,filename='loop'):

		if self._operationMode.control:
			len_window = 4096 #Specified length of analysis window
			hop_length = 1024 #Specified percentage hop length between windows
			t = time()
			n_frames = 100#n_frames
			mag_buffer = np.zeros((self.topology.output_size,1))
			activations = values[:,0:8]
			for ii in range(n_frames):
				orig_hat = np.reshape(self._sess.run(self.outputLayer2,feed_dict={self.controller:activations}),[self.topology.output_size,-1])
	...
	def play_synth(self,values, n_frames=300,LFO=0,filename='loop.wav'):
		...
		for ii in range(n_frames):
			...
			mag_buffer = np.hstack((mag_buffer,orig_hat))		
		if LFO>0:
			current_activations = np.zeros((1,8))
			mag_buffer = np.zeros((self.topology.output_size,1))
			lfo_freq = LFO
			for ii in range(n_frames):
				current_activations[:,0:2] = activations[:,0:2]*(1.0+0.25*np.sin(2*np.pi*ii*lfo_freq/20000))
				current_activations[:,2:4] = activations[:,2:4]*(1.0+0.25*np.sin(2*np.pi*ii*lfo_freq*0.707107/20000))
				current_activations[:,4:6] = activations[:,4:6]*(1.0+0.25*np.sin(2*np.pi*ii*lfo_freq*0.415253/20000))
				current_activations[:,6:] = activations[:,6:]*(1.0+0.25*np.sin(2*np.pi*ii*lfo_freq*0.19258/20000))
		...
		sf.write(filename, V, 44100, subtype='PCM_16')  #Must be 16bit PCM to work with pygame
